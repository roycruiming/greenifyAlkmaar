...
namespace CafofoStudio
{
    ...
    public class SoundSubElementSample : ISoundSubElement
    {
        ...
        public void CalculateIntensity(float intensity, float volumeMultiplier)
        {
            ...
            float fMin = Mathf.Lerp(minSampleFrequency, maxSampleFrequency, intensity);
            float fMax = Mathf.Lerp(minSampleFrequency + minSampleFrequencyDelta, maxSampleFrequency + maxSampleFrequencyDelta, intensity);
            float currentFrequency = Random.Range(1 / fMin, 1 / fMax);
            nextSampleCountdown = currentFrequency;
        }
        ...
        private void PlayAnySample(float volumeMultiplier)
        {
            ...
            tempSource.clip = audioClips[Random.Range(0, audioClips.Count)];
            tempSource.volume = Random.Range(0.2f, 1) * volumeMultiplier;
